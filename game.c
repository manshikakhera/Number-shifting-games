...
int readEnteredKey()
{
    char c;
    c = _getch();
    if (c == -32)//press any key that is why we use -32
        c = _getch();
...
void gameRule(int arr[][4])
{
    ...
    for (i = 0; i < 4; i++)
    {
        printf(ANSI_COLOR_YELLOW "| " ANSI_COLOR_RESET); //left line is printing 
        for (j = 0; j < 4; j++)
        {
            if (arr[i][j] != 0)
                printf(ANSI_COLOR_YELLOW "%2d | " ANSI_COLOR_RESET, 4 * i + j + 1); // numbers are printing
            else
                printf(ANSI_COLOR_YELLOW "   |" ANSI_COLOR_RESET);//blank space is printting
        }
        printf("\n");
    }
